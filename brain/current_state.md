...
- Customer inquiries: **`/contact`** = general help (FAQ + accounts/billing/subscriptions form). **`/regulation-help`** = ask a regulation question or suggest coverage (with autocomplete). Homepage/search **Questions or Suggestions?** links to **`/regulation-help`** (button **Go**); copy explicitly routes account/billing/subscription questions to linked **Contact** (`/contact`). Regulation detail pages include a bottom question panel (same DB limits as other forms).
- New `/pricing` page is live with marketing content for Free/Professional/Enterprise tiers; route is wired and footer now includes a **Pricing** link.
